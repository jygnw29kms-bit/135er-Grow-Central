...
def main() -> int:
    ...
    try:
        ...
        run("nmcli", "connection", "down", AP_CONNECTION, check=False)
        run("ufw", "--force", "delete", "allow", "from", "10.42.0.0/24", "to", "any", "port", "80", "proto", "tcp", check=False)
        run("ufw", "--force", "delete", "allow", "from", "10.42.0.0/24", "to", "any", "port", "443", "proto", "tcp", check=False)
        time.sleep(2)
