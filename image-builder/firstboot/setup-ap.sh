...
# Pi 3B uses BCM43430 (2.4 GHz only). Explicitly clear rfkill and establish
# the regulatory domain because Pi 4 hardware is more forgiving here.
rfkill unblock wifi || true
nmcli radio wifi on || true
iw reg set "$REGDOMAIN" || true

# brcmfmac can appear a few seconds after NetworkManager during a cold boot.
...
# stale MAC/802.1X/security state that can make a Pi-4-tested profile unsuitable
# for the BCM43430 in a Raspberry Pi 3B.
