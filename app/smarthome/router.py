...
from .fritz_onboarding import router as fritz_onboarding_router
...
async def list_devices():
    return [PublicDevice(id=item.id, name=item.name, adapter=item.adapter, capability=item.capability, approved=item.approved, writable=item.writable, metadata=item.metadata) for item in _registry().list()]
