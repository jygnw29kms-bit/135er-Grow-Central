...
function updateRole(){
 ...
 const txt=currentUser.role==='admin'?'Administrator – Termine + Benutzerverwaltung':currentUser.role==='editor'?'Bearbeitung – Termine anlegen/ändern/verschieben':'Nur Lesen';
 $('roleBadge').textContent=`${currentUser.name}: ${txt}`;
...
async function loadUsers(){
 try{
   ...
   d.users.forEach(u=>list.appendChild(userRow(u)));
...
function userRow(u){
 ...
 row.innerHTML=`
   ...
   <div class="user-edit-grid">
     <label>Name<input class="u-name" value="${esc(u.name)}"></label>
     <label>Rolle<select class="u-role"><option value="viewer">Nur Lesen</option><option value="editor">Bearbeitung</option><option value="admin">Administrator</option></select></label>
   ...
   <div class="row-actions"><span class="row-error"></span><button type="button" class="save-user primary">Speichern</button></div>`;
 ...
 row.querySelector('.save-user').onclick=async()=>{
   ...
   try{await api('/api/users/'+u.id,{method:'PUT',body:JSON.stringify(payload)});toast('Benutzer gespeichert');await loadUsers();if(u.id===currentUser.id){const me=await api('/api/me');currentUser=me.user;updateRole()}}
...
async function createUser(e){
 ...
 try{
   await api('/api/users',{method:'POST',body:JSON.stringify({name:$('newUserName').value.trim(),role:$('newUserRole').value,password:$('newUserPassword').value})});
   $('newUserForm').reset();$('newUserRole').value='viewer';toast('Benutzer angelegt');await loadUsers();
