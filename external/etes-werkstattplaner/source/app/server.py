...
def verify_password(password, stored):
    try:
        ...
        salt = base64.b64decode(salt64); expected = base64.b64decode(dig64)
...
def upsert_user(name, role, password):
    if role not in ('admin','editor','viewer'): raise ValueError('Ungültige Rolle')
    if len(password) < 10: raise ValueError('Passwort muss mindestens 10 Zeichen haben')
    con = db_connect()
...
def validate_appt(b, forced_id=None):
    ...
    if tmins(a['end']) <= tmins(a['start']): raise ValueError('Endzeit muss nach Startzeit liegen')
...
class Handler(SimpleHTTPRequestHandler):
    server_version = 'EtesWerkstattplaner/1.0'
    ...
    def json_out(self,obj,status=200,extra=None):
        raw=json.dumps(obj,ensure_ascii=False).encode(); self.send_response(status); self.send_header('Content-Type','application/json; charset=utf-8'); self.send_header('Content-Length',str(len(raw)))
        if extra:
    ...
    def read_json(self):
        try:
            ...
            if n>262144: raise ValueError('Anfrage zu groß')
            return json.loads(self.rfile.read(n) or b'{}')
        except Exception: raise ValueError('Ungültiges JSON')
    def session_user(self):
        c=SimpleCookie(self.headers.get('Cookie','')); morsel=c.get('etes_session')
        ...
        with SESSIONS_LOCK:
            ...
            s['expires']=time.time()+12*3600; return s['user']
    def require(self,roles=None):
        u=self.session_user()
        if not u: self.json_out({'error':'Nicht angemeldet'},401); return None
        if roles and u['role'] not in roles: self.json_out({'error':'Keine Berechtigung'},403); return None
        return u
    ...
    def do_GET(self):
        ...
        if p.path=='/health': return self.json_out({'ok':True,'service':'etes-werkstattplaner'})
        if p.path.startswith('/api/'):
            if p.path=='/api/me':
                u=self.require(); return None if not u else self.json_out({'user':u})
            if p.path=='/api/config':
                u=self.require(); return None if not u else self.json_out({'resources':RESOURCES,'statuses':STATUSES,'mechanics':MECHANICS,'parts':PARTS,'loaner':LOANER})
            if p.path=='/api/appointments':
                u=self.require();
                if not u:return
                date=clean(parse_qs(p.query).get('date',[''])[0],10)
                if not valid_date(date): return self.json_out({'error':'Ungültiges Datum'},400)
                con=db_connect(); rows=con.execute('''SELECT a.*,cu.name created_by_name,uu.name updated_by_name FROM appointments a LEFT JOIN users cu ON a.created_by=cu.id LEFT JOIN users uu ON a.updated_by=uu.id WHERE a.date=? ORDER BY a.start,a.resource''',(date,)).fetchall(); con.close()
                return self.json_out({'appointments':[dict(r) for r in rows]})
            if p.path=='/api/users':
                u=self.require(('admin',));
                if not u:return
                con=db_connect(); rows=con.execute('SELECT id,name,role,active,created_at,updated_at FROM users ORDER BY name').fetchall(); con.close(); return self.json_out({'users':[dict(r) for r in rows]})
            if p.path=='/api/events': return self.handle_events()
            return self.json_out({'error':'Nicht gefunden'},404)
        if p.path=='/': self.path='/index.html'
        return super().do_GET()
    def do_POST(self):
        if not self.safe_origin(): return self.json_out({'error':'Ungültiger Origin'},403)
        p=urlparse(self.path)
        if p.path=='/api/login':
            try:b=self.read_json()
            except ValueError as e:return self.json_out({'error':str(e)},400)
            name=clean(b.get('name'),80); pw=str(b.get('password') or '')
            con=db_connect(); row=con.execute('SELECT * FROM users WHERE name=? AND active=1',(name,)).fetchone(); con.close()
            if not row or not verify_password(pw,row['password_hash']): return self.json_out({'error':'Name oder Passwort falsch'},401)
            token=secrets.token_urlsafe(32); user={'id':row['id'],'name':row['name'],'role':row['role']}
            with SESSIONS_LOCK: SESSIONS[token]={'user':user,'expires':time.time()+12*3600}
        ...
        if p.path=='/api/logout':
            u=self.require();
            if not u:return
            c=SimpleCookie(self.headers.get('Cookie','')); m=c.get('etes_session')
            if m:
                with SESSIONS_LOCK: SESSIONS.pop(m.value,None)
        ...
        if p.path=='/api/appointments':
            ...
            con=db_connect(); conflict=con.execute('SELECT id,plate FROM appointments WHERE id<>? AND date=? AND resource=? AND start<? AND end>? LIMIT 1',(a['id'],a['date'],a['resource'],a['end'],a['start'])).fetchone()
            if conflict and not b.get('force'): con.close(); return self.json_out({'error':'Arbeitsplatz bereits belegt','conflict':dict(conflict)},409)
            ...
            con.execute('INSERT INTO audit_log(user_id,action,appointment_id,details) VALUES(?,?,?,?)',(u['id'],'create',a['id'],f"{a['date']} {a['start']}-{a['end']} {a['resource']}")); con.commit(); con.close(); notify_change(); return self.json_out({'ok':True},201)
        return self.json_out({'error':'Nicht gefunden'},404)
    def do_PUT(self):
        if not self.safe_origin(): return self.json_out({'error':'Ungültiger Origin'},403)
        m=re.fullmatch(r'/api/appointments/([^/]+)',urlparse(self.path).path)
        if not m:return self.json_out({'error':'Nicht gefunden'},404)
        u=self.require(('admin','editor'));
        if not u:return
        try:b=self.read_json(); aid=m.group(1); a=validate_appt(b,aid)
        except ValueError as e:return self.json_out({'error':str(e)},400)
        con=db_connect(); old=con.execute('SELECT * FROM appointments WHERE id=?',(aid,)).fetchone()
        if not old: con.close(); return self.json_out({'error':'Termin nicht gefunden'},404)
        ...
        con.execute('INSERT INTO audit_log(user_id,action,appointment_id,details) VALUES(?,?,?,?)',(u['id'],'update',aid,f"{old['date']}/{old['resource']}/{old['start']} -> {a['date']}/{a['resource']}/{a['start']}")); con.commit(); con.close(); notify_change(); return self.json_out({'ok':True})
    def do_DELETE(self):
        if not self.safe_origin(): return self.json_out({'error':'Ungültiger Origin'},403)
        m=re.fullmatch(r'/api/appointments/([^/]+)',urlparse(self.path).path)
        if not m:return self.json_out({'error':'Nicht gefunden'},404)
        u=self.require(('admin','editor'));
        if not u:return
        aid=m.group(1); con=db_connect(); old=con.execute('SELECT * FROM appointments WHERE id=?',(aid,)).fetchone()
        if not old: con.close(); return self.json_out({'error':'Termin nicht gefunden'},404)
        con.execute('DELETE FROM appointments WHERE id=?',(aid,)); con.execute('INSERT INTO audit_log(user_id,action,appointment_id,details) VALUES(?,?,?,?)',(u['id'],'delete',aid,f"{old['date']} {old['start']}-{old['end']} {old['plate']}")); con.commit(); con.close(); notify_change(); return self.json_out({'ok':True})
    def handle_events(self):
        u=self.session_user()
        if not u:return self.json_out({'error':'Nicht angemeldet'},401)
        ...
        try:
            ...
            while True:
                with EVENT_COND: EVENT_COND.wait(timeout=20); current=EVENT_VERSION
                if current!=seen:
                    seen=current; self.wfile.write(b'event: appointments-changed\ndata: {}\n\n')
...
def main():
    ...
    u=sub.add_parser('user'); u.add_argument('name');u.add_argument('role',choices=['admin','editor','viewer']);u.add_argument('password',nargs='?')
    args=ap.parse_args()
    if args.cmd=='user':
        import getpass
        pw=args.password or getpass.getpass(f'Passwort für {args.name}: ')
        try:upsert_user(args.name,args.role,pw);print(f'Benutzer {args.name} als {args.role} angelegt/aktualisiert.')
        except ValueError as e:print(e,file=sys.stderr);sys.exit(2)
        return
    print(f"Ete's Werkstattplaner auf http://{HOST}:{PORT}")
